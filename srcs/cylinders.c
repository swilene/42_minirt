...
/*   Updated: 2023/03/14 16:54:41 by saguesse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculs_cylinders(t_data *data)
{
	...
	while (tmp)
	{
		if (!strncmp("cy", tmp->identifier, 2))
		{
			...
			tmp->ra0 = cross_product(cross_product(tmp->s, sub(data->c.coord, tmp->ra1)), tmp->s);
...
int	render_cylinders(t_obj *tmp, t_vector ray)
{
	...
	if (t1 > 0)
		...
		tmp->t = t2;
	if (dot_product(sub(ray, tmp->ra1), tmp->s) > 0
			&& dot_product(sub(ray, tmp->ra2), tmp->s) < 0)
		return (0);
	return (1);
}
